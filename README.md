...
- `qwdtt-client` - клиент на Go. Создает TUN-интерфейс, названный по секции,
...
секция `qwdtt0`; вторая секция - это второй туннель, и ей нужны свои
`route_table`, `rule_priority` и метка `fwmark`, иначе init-скрипт ее не
запустит и напишет причину в журнал.

Имя секции - это имя TUN-устройства, как имя интерфейса в
`/etc/config/network`: секция `qwdtt0` поднимает `qwdtt0`. Отсюда ограничение
в 15 символов и смысл префикса - зона `qwdtt` в firewall принимает устройства
по шаблону `qwdtt+`, поэтому имя, которое начинается с `qwdtt`, попадает в нее
само, а имя с другим началом нужно добавить в зону вручную.

`lan_interface` по умолчанию - `br-lan`. Если в вашей сборке OpenWrt LAN
называется иначе, поменяйте это поле.

1. Задайте адрес сервера, пароль и хеши звонка и включите туннель - через
   LuCI (Services -> qWDTT) или через UCI:

   ```sh
   uci set qwdtt.qwdtt0.peer_host='IP_АДРЕС_СЕРВЕРА'
   uci set qwdtt.qwdtt0.peer_port='56003'
   uci set qwdtt.qwdtt0.password='ПАРОЛЬ'
   uci add_list qwdtt.qwdtt0.hash='ХЕШ_ЗВОНКА'
...
2. Включите автозапуск сервиса и запустите туннель - кнопкой Restart в его
   строке на странице Services -> qWDTT в LuCI или из шелла:
...
```sh
/usr/bin/qwdtt -s qwdtt0 stop
/etc/init.d/qwdtt stop
```

Это только текущее состояние: вернется ли туннель после перезагрузки, решает
его `enabled`, и эти команды его не трогают.
...
`iperf3 --bind-dev qwdtt0`), переживает переподключение, а не обрывается
намертво - иначе сокет остался бы привязан к номеру интерфейса, которого больше
нет.

Обратная сторона: пока клиент не подключился, трафик, направленный в туннель,
отбрасывается, а не уходит напрямую через WAN. Это kill switch - лучше остаться
без интернета, чем выпустить трафик мимо туннеля.

Устройство убирается только когда туннель останавливают (`qwdtt -s qwdtt0 stop`
или кнопка Stop) или когда секцию выключают (`enabled '0'`) и применяют
настройки. Вместе с ним уходят и маршруты, и трафик снова идет напрямую.
