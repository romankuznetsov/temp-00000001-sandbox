...
# One catch-all tunnel and one selected by mark, which is the shape the retired
# init script allowed and the documentation described. work is switched off and
# names a LAN that is not br-lan, so it also covers the two things the
# conversion must not do quietly.
...
# shows up here rather than on somebody's router.
got=$(dump)
want='network.qwdtt0.hash=aaa bbb
...
network.qwdtt0_killswitch.metric=4096
...
network.qwdtt0_rule.priority=10000
...
network.work_killswitch.metric=4096
